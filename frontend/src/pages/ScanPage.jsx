...
function EmotionAgeBadges({ emotion, age, light = false }) {
  const Badge = ({ title, label, confidence }) => (
    <div
      className={`flex-1 rounded-xl p-3 text-center border ${light ? "bg-white border-gray-200" : "bg-[#1a1a2e] border-gray-700"}`}
    >
      <div
        className={`text-xs mb-1 ${light ? "text-gray-500" : "text-gray-400"}`}
      >
        {title}
      </div>
      <div
        className={`font-semibold capitalize ${light ? "text-gray-900" : "text-white"}`}
      >
        {label ?? "-"}
      </div>
      {confidence != null && (
        <div
          className={`text-xs mt-0.5 ${light ? "text-blue-600" : "text-blue-400"}`}
        >
          {confidence}%
        </div>
      )}
  ...
  return (
    <div className="flex gap-3">
      <Badge
        ...
        label={emotion?.class_name}
        confidence={emotion?.confidence}
      />
      <Badge
        title="Estimated Age:"
        label={age?.class_name}
...
function buildAssistantScanContext(result) {
  ...
  if (scanType === "breed") {
    return {
      ...
      age: result?.age ?? null,
      top_breeds: (Array.isArray(result?.top_breeds)
        ? result.top_breeds
        : []
      ).map((breed) => ({
  ...
  return {
    scan_type: "disease",
    uploaded_image_url: result?.uploaded_image_url || null,
    top_diseases: (Array.isArray(result?.top_diseases)
      ? result.top_diseases
      : []
    ).map((disease) => ({
...
export function ScanWorkspace({
  ...
  const updatePublicUsage = useCallback(
    (payload) => {
  ...
  async function handleScan() {
    if (!file || loading || (publicMode && publicUsage?.demo_remaining === 0))
      return;
    ...
    try {
      ...
      const endpoint = usePublicEndpoint
        ? activeMode.publicEndpoint
        : activeMode.endpoint;
      const headers = usePublicEndpoint ? {} : { ...getAuthHeaders() };

      const { data } = await api.post(endpoint, form, { headers });
      updatePublicUsage(data);
      setResult(data);
      setExpandedBreedId(null);
      setBreedDetails({});
  ...
  async function handleBreedTap(breedId) {
    ...
    try {
      const requestConfig = publicMode
        ? undefined
        : { headers: getAuthHeaders() };
      const { data } = await api.get(
        `/api/scans/breed/${breedId}`,
        requestConfig,
      );
  ...
  const buildHistoryPayload = useCallback(() => {
    ...
    if (result.scan_type === "breed") {
      ...
      const topDiseases = Array.isArray(result.top_diseases)
        ? result.top_diseases
        : [];
      ...
      predictions = topDiseases.map((disease, idx) => ({
        confidence: Number(
          disease?.mix_share != null
            ...
            : totalDiseaseConfidence > 0
              ? (Number(disease?.confidence ?? 0) / totalDiseaseConfidence) *
                100
              : 0,
        ),
        rank: idx + 1,
        breed_id: null,
        class_name:
          disease?.class_name ?? disease?.display_name ?? `disease_${idx + 1}`,
        display_name:
          disease?.display_name ?? disease?.class_name ?? `Disease ${idx + 1}`,
      }));
    }
    ...
    const imageUrl =
      typeof result?.uploaded_image_url === "string"
  ...
  async function handleSaveToHistory() {
    ...
    try {
      if (payload.image_url) {
        ...
        form.append(
          "share_for_training",
          payload.share_for_training ? "true" : "false",
        );
      ...
      if (err?.response?.status === 401) {
        ...
        const message =
          err?.response?.data?.error ||
          "Failed to save history. You can try again.";
    ...
    <div className="mt-4">
      {result?.scan_type === "breed" && (
        <div className="mb-3">
          ...
          <p
            className={`mt-2 text-xs ${isLight ? "text-gray-600" : "text-gray-400"}`}
          >
            Allow admin to review and use this scan image and breed result to
            improve the system.
          </p>
        </div>
      )}
      ...
      {historySaveState === "error" && (
        <p
          className={`mt-2 text-xs ${isLight ? "text-red-600" : "text-red-400"}`}
        >
  ...
  return (
    <div
      className={
        inModal
          ? "text-gray-900"
          : "min-h-screen bg-[#0f0f1a] text-white p-4 md:p-8 max-w-2xl mx-auto"
      }
    >
      <div className={inModal ? "max-w-2xl mx-auto p-4 md:p-6" : ""}>
        ...
        {publicMode && publicUsage && (
          <div
            className={`mb-4 rounded-lg border px-3 py-2 text-sm ${isLight ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-blue-900/30 border-blue-700 text-blue-200"}`}
          >
            Free demo scans left on this device:{" "}
            <span className="font-semibold">{publicUsage.demo_remaining}</span>{" "}
            / {publicUsage.demo_limit}
          </div>
        )}
        {mode === "breed" ? (
          <p
            className={`text-xs mb-2 ${isLight ? "text-gray-500" : "text-gray-400"}`}
          >
            For best results, ensure the image clearly shows the dog's face and
            physical traits.
          </p>
        ):
        <p
            className={`text-xs mb-2 ${isLight ? "text-gray-500" : "text-gray-400"}`}
          >
            For best results, ensure the image clearly shows the dog's skin and
            the possible skin disease.
          </p>
          }
        <div className="mb-4">
          <input
            ...
            {preview ? (
              ...
              <div className="py-8 select-none">
                <div className="flex justify-center mb-3">
                  <Upload
                    className={`w-12 h-12 ${publicMode ? "text-blue-500" : "text-blue-400"}`}
                  />
                </div>
                ...
                <p
                  className={`text-xs mt-1 ${isLight ? "text-gray-500" : "text-gray-600"}`}
                >
        ...
        {publicMode && publicUsage?.demo_remaining === 0 && (
          <p
            className={`text-sm mb-4 ${isLight ? "text-red-600" : "text-red-300"}`}
          >
            Demo limit reached on this device. Sign up to keep scanning and save
            results.
          </p>
        )}
        ...
        {error && (
          <div
            className={`rounded-xl p-4 mb-4 text-sm flex items-start gap-2 ${isLight ? "bg-red-50 border border-red-200 text-red-700" : "bg-red-900/40 border border-red-700 text-red-300"}`}
          >
            ...
            {result.scan_type === "breed" && (
              <>
                ...
                <div>
                  ...
                  <p
                    className={`text-xs mt-0.5 mb-3 ${isLight ? "text-gray-500" : "text-gray-500"}`}
                  >
                    Tap a card to view full breed profile
                  </p>
                  <div className="space-y-3">
                    {(Array.isArray(result.top_breeds)
                      ? result.top_breeds
                      : []
                    ).map((breed, i) => (
                              ...
                              <div className="space-y-3">
                                <div className="grid grid-cols-2 gap-3 text-sm">
                                  <div>
                                    ...
                                    >
                                      {breedDetails[breed.breed_id].size ||
                                        "Unknown"}
                                    </span>
                                  </div>
                                  <div>
                                ...
                                >
                                  {breedDetails[breed.breed_id].description ||
                                    "No description available."}
                                </p>

                                <div>
                                  <p
                                    className={`text-xs font-semibold mb-2 ${isLight ? "text-gray-600" : "text-gray-300"}`}
                                  >
                                    Physical Traits
                                  </p>
                                  <div className="grid grid-cols-2 gap-2 text-sm">
                                    <div
                                      className={`rounded-lg px-3 py-2 ${isLight ? "bg-gray-50 text-gray-800" : "bg-[#111124] text-gray-200"}`}
                                    >
                                      <span
                                        className={
                                          isLight
                                      ...
                                      {breedDetails[breed.breed_id].snout ||
                                        "Unknown"}
                                    </div>
                                    <div
                                      className={`rounded-lg px-3 py-2 ${isLight ? "bg-gray-50 text-gray-800" : "bg-[#111124] text-gray-200"}`}
                                    >
                                      ...
                                      {breedDetails[breed.breed_id].ears ||
                                        "Unknown"}
                                    </div>
                                    <div
                                      className={`rounded-lg px-3 py-2 ${isLight ? "bg-gray-50 text-gray-800" : "bg-[#111124] text-gray-200"}`}
                                    >
                                      ...
                                      {breedDetails[breed.breed_id].coat ||
                                        "Unknown"}
                                    </div>
                                    <div
                                      className={`rounded-lg px-3 py-2 ${isLight ? "bg-gray-50 text-gray-800" : "bg-[#111124] text-gray-200"}`}
                                    >
                                      ...
                                      {breedDetails[breed.breed_id].tail ||
                                        "Unknown"}
                                    </div>
                                  </div>
                                </div>
                                ...
                                <div>
                                  ...
                                  <div className="grid grid-cols-2 gap-3 text-sm">
                                    ...
                                    <div>
                                      ...
                                      >
                                        {breedDetails[breed.breed_id].origin ||
                                          "Unknown"}
                                      </span>
                                    </div>
                                    <div>
                                ...
                                <div>
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {renderTemperament(
                                      breedDetails[breed.breed_id].temperament,
                                    ).map((temp) => (
                                      <span
                                        key={temp}
                                ...
                                {breedDetails[breed.breed_id]
                                  ...
                                  <div>
                                    <p
                                      className={`text-xs font-semibold mb-1 ${isLight ? "text-gray-600" : "text-gray-300"}`}
                                    >
                                ...
                                {breedDetails[breed.breed_id]
                                  .key_health_tips && (
                                  <div
                                    className={`rounded-lg px-3 py-2 text-sm ${isLight ? "bg-green-50 text-green-800 border border-green-200" : "bg-green-950/30 text-green-300 border border-green-900"}`}
                                  >
                                ...
                                <div className="pt-1">
                                  <button
                                    onClick={() =>
                                      window.location.assign(`/doglibrary`)
                                    }
            ...
            {result.scan_type === "disease" && (
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5 mb-3">
                  For reference only. Consult a veterinarian for diagnosis.
                </p>
                <div className="space-y-3">
                  {(Array.isArray(result.top_diseases)
                    ? result.top_diseases
                    : []
                  ).map((disease, i) => (
